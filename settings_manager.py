...
class SettingsManager:
    ...
    # ... (reload_default_tag_mappings без змін) ...
    ...
    def load_settings(self):
        ...
        default_settings_values = {
            ...
            "bracket_tag_color_hex": getattr(self.mw, 'bracket_tag_color_hex', "#FF8C00"), # Оновлено
            "critical_problem_lines_per_block": {}, 
            "warning_problem_lines_per_block": {}
        }
        # ... (решта load_settings без змін) ...
        settings_data = {}
        if not os.path.exists(self.settings_file_path):
            ...
            try:
                ...
                self.mw.bracket_tag_color_hex = settings_data.get("bracket_tag_color_hex", default_settings_values["bracket_tag_color_hex"]) # Оновлено
                ...
                if isinstance(crit_problems, dict): self.mw.critical_problem_lines_per_block = {k: set(v) for k, v in crit_problems.items() if isinstance(v, list)}
                ...
                if isinstance(warn_problems, dict): self.mw.warning_problem_lines_per_block = {k: set(v) for k, v in warn_problems.items() if isinstance(v, list)}
    ...
    def save_settings(self):
        # ... (решта save_settings без змін, bracket_tag_color_hex буде зберігатися з self.mw) ...
        ...
        log_debug(f"  Saving current default_tag_mappings from program memory (count: {len(self.mw.default_tag_mappings)}).")
        ...
        settings_data["editors_wrap_lines"] = self.mw.editors_wrap_lines; settings_data["bracket_tag_color_hex"] = self.mw.bracket_tag_color_hex # Зберігаємо поточне значення
