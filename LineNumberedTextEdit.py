...
class LineNumberedTextEdit(QPlainTextEdit):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self.isReadOnly() and hasattr(actual_main_window, 'original_text_edit') and self == actual_main_window.original_text_edit:
                ...
                if tag_text:
                    ...
                    log_debug(f"LNET ({self.widget_id} - original_text_edit): Copied tag: {tag_text}")
                ...
                if clicked_tag_text:
                    ...
                    if re.fullmatch(r"\{[^}]*\}", clipboard_text):
                        log_debug(f"LNET ({self.widget_id} - edited_text_edit): Replacing '{clicked_tag_text}' with '{clipboard_text}'.")
    ...
    def setLinkedCursorPosition(self, line_number: int, column_number: int):
        ...
        if line_number >= 0 and line_number < doc.blockCount():
            ...
            if block.isValid():
                line_sel = self._create_block_background_selection(block, self.linked_cursor_block_color, use_full_width=False) # Тепер False
                ...
                if temp_cursor_highlight.hasSelection() or \
                   (actual_column == line_text_length and not temp_cursor_highlight.hasSelection() and line_text_length > 0): # Умова для кінця рядка
    ...
    def setPreviewSelectedLineHighlight(self, line_number: int):
        ...
        if line_number >= 0 and line_number < doc.blockCount():
            block = doc.findBlockByNumber(line_number)
            # Використовуємо use_full_width=False, щоб підсвічувався весь перенесений блок
            selection = self._create_block_background_selection(block, self.preview_selected_line_color, use_full_width=False)
            if selection:
                new_selections.append(selection)
        ...
        if line_number >= 0 and line_number < doc.blockCount():
            ...
            if block.isValid():
                ...
                if not is_already_added:
                    # Використовуємо use_full_width=False для проблемних рядків
                    selection = self._create_block_background_selection(block, self.problem_line_color, use_full_width=False)
                    if selection:
    ...
    def removeProblemLineHighlight(self, line_number: int) -> bool:
        removed = False
        initial_len = len(self._problem_line_selections)
        self._problem_line_selections = [
            s for s in self._problem_line_selections if s.cursor.blockNumber() != line_number
        ]
        if len(self._problem_line_selections) < initial_len:
            log_debug(f"LNET ({self.widget_id}): Marked line {line_number} for removal from problem highlights. Problems left in queue: {len(self._problem_line_selections)}")
            removed = True
        return removed
    ...
    def clearProblemLineHighlights(self):
        needs_update = bool(self._problem_line_selections)
        if needs_update:
            log_debug(f"LNET ({self.widget_id}): clearProblemLineHighlights. Had {len(self._problem_line_selections)} problems.")
        self._problem_line_selections = []
        if needs_update: # Застосовуємо тільки якщо щось змінилося
            self._apply_all_extra_selections()

    def hasProblemHighlight(self, line_number: Optional[int] = None) -> bool:
        if line_number is not None:
            return any(s.cursor.blockNumber() == line_number for s in self._problem_line_selections)
