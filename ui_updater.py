...
class UIUpdater:
    def __init__(self, main_window, data_processor):
        ...
        self.problem_block_color = QColor(Qt.yellow).lighter(150) # Для критичних проблем
        self.warning_block_color = QColor(Qt.lightGray).lighter(110) # Для попереджень (трохи темніший за фон рядка)
    ...
    def populate_blocks(self):
        ...
        for i in range(len(self.mw.data)):
            ...
            # Пріоритет підсвічування блоку: критична проблема > попередження
            if num_critical > 0:
                self.highlight_problem_block(i, True, is_critical=True)
            elif num_warnings > 0:
                self.highlight_problem_block(i, True, is_critical=False) # Попередження
            else: 
    ...
    def populate_strings_for_block(self, block_idx):
        ...
        if preview_edit:
            ...
            if hasattr(preview_edit, 'applyQueuedHighlights'): # Використовуємо новий загальний метод
            ...
            if self.mw.current_string_idx != -1 and \
               ...
               self.mw.current_string_idx < preview_edit.document().blockCount(): 
                preview_edit.setPreviewSelectedLineHighlight(self.mw.current_string_idx)
                 ...
                 preview_edit._apply_all_extra_selections() # Для певності, якщо не було setPreviewSelectedLineHighlight
    ...
    def clear_status_bar(self):
        ...
        if hasattr(self.mw, 'pos_len_label') and self.mw.pos_len_label:
            self.mw.pos_len_label.setText("0/0")
    ...
    def update_text_views(self): 
        # ... (код без змін, як у попередньому повідомленні, з виправленням для original_text_raw)
        ...
        if self.mw.current_block_idx != -1 and self.mw.current_string_idx != -1:
            ...
            log_debug("  No active block/string selected. Text views will be cleared.")
