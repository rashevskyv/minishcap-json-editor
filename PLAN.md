...
5.  **Взаємодія з рядками: Множинне виділення у прев'ю**
    ...
    *   **Стан Реалізації (Завершено):**
        *   **Проблема:** Початковий підхід з використанням `ExtraSelections` для візуального виділення виявився ненадійним. Цей механізм є тимчасовим, і будь-яка дія (клік мишею, навігація) призводила до того, що система скидала візуальне виділення, хоча логіка вибору рядків працювала коректно.
        *   **Рішення:** Було прийнято рішення кардинально змінити стратегію. Замість тимчасової підсвітки, тепер **напряму змінюється колір фону** текстових блоків.
            1.  **Пряма зміна фону:** У `TextHighlightManager` додано новий механізм, який змінює властивість `background` у `QTextBlockFormat` для кожного виділеного рядка. Це постійна зміна, яка не скидається при інших діях.
            2.  **Ефективне оновлення:** У `LineNumberedTextEdit` реалізовано логіку, яка відстежує різницю між попереднім та поточним станом виділення, що дозволяє перемальовувати фон лише для тих рядків, чий статус змінився.
            3.  **Ізольована обробка подій:** Обробники подій миші (`mousePressEvent`, `mouseReleaseEvent`) для віджета прев'ю були повністю ізольовані, щоб запобігти конфліктам з базовою логікою `QPlainTextEdit`.
        *   **Результат:** Новий підхід є надійним, стабільним і забезпечує коректне візуальне відображення мульти-виділення за допомогою `Ctrl` та `Shift` за будь-яких умов.
    *   **Файли та зміни:**
        *   **Змінено `components/LineNumberedTextEdit.py`, `components/TextHighlightManager.py`, `components/LNET_mouse_handlers.py`, `ui/ui_event_filters.py`**: Реалізовано повну логіку мульти-виділення, обробки подій та візуалізації.
