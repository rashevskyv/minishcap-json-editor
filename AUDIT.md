...
### Оновлення v0.2.8 (2026-03-18) — Hotfix Release

Виконано комплексне виправлення критичних та UI багів:

1.  **Логіка маркування пробілів**: Виправлено `utils.py`, щоб поодинокі пробіли всередині тексту не перетворювалися на крапки. Тепер крапки показуються лише для: `2+` пробілів поспіль, або будь-якої кількості пробілів на початку/в кінці рядка (враховуючи `\r\n` для Windows).
2.  **По-рядкова індикація змін (`*`)**: Оновлено `line_number_area_paint_logic.py`. Зірочка в прев'ю тепер ставиться лише навпроти конкретного зміненого рядка, а не всього стрінга.
3.  **Виправлення `[DATA ERROR]`**: У `DataStateProcessor` додано boundary checks. Якщо дані ще завантажуються або індекси тимчасово невалідні, повертається пустий рядок замість тексту помилки.
4.  **Персистентність останнього проєкту**: Виправлено баг у `closeEvent`, через який налаштування (геометрія вікна та шлях до проєкту) не зберігалися при наявності незбережених змін у тексті. Тепер `last_opened_path` оновлюється при кожному відкритті проєкту.
5.  **Синхронізація UI при старті**: Додано `QTimer` затримку перед відновленням стану (вибір блоку/рядка) при запуску програми, щоб дерево блоків встигло повністю проініціалізуватися. Це усунуло проблему "порожнього екрану" при автозавантаженні.
6.  **Видимість кнопок категорій**: Налаштування "Highlight/Hide moved" тепер відображаються лише для батьківських блоків, що мають віртуальні підблоки (категорії).
